...
#include "rdpartyrobotcdr-3.1/drivers/hitechnic-touchmux.h"
...
task main()
{
	...
	while (true)
	{
		int shoulderPower = 0;
		int elbowPower = 0;
		int switches = SensorValue[S1];
		writeDebugStreamLine("%d",switches);
		if (switches & 0x8)
			shoulderPower = -50;
		...
		nxtDisplayTextLine(1,"Shoulder: %d",nMotorEncoder[shoulder]);
		nxtDisplayTextLine(2,"Elbow:    %d",nMotorEncoder[elbow]);
		nxtDisplayTextLine(3,"Wrist:    %d",nMotorEncoder[wrist]);
		nxtDisplayTextLine(4,"<Wrist:   %f",encoderToDegrees(jWrist, nMotorEncoder[wrist]));
		nxtDisplayTextLine(5,"<Elbow:   %f",encoderToDegrees(jElbow, nMotorEncoder[elbow]));
		nxtDisplayTextLine(6,"<Shoulder:%f",encoderToDegrees(jShoulder, nMotorEncoder[shoulder]));
