...
void driveTowardsBall()
{
  setLeftDrive(45);
  setRightDrive(45);
  wait1Msec(DRIVE_TOWARDS_BALL_TIME);
  stopDrive();
}
...
bool runUltrasonic()
{
  ...
  setLeftDrive(45);
  setRightDrive(-45);
  while (timeMark + ULTRASONIC_ROTATION_TIME > nPgmTime)
  {
  ...
  setLeftDrive(-45);
  setRightDrive(45);
  while (timeMark + 2*ULTRASONIC_ROTATION_TIME > nPgmTime)
  {
    if (SensorValue[sSONAR] < BB_THRESHOLD_FAR)
    {
      wait1Msec(ULTRASONIC_DELAY);
  ...
  setLeftDrive(45);
  setRightDrive(-45);
  wait1Msec(ULTRASONIC_ROTATION_TIME);
  stopDrive();
  return false;
}
...
void headForCorner()
{
  ...
  setRightDrive(45);
  setLeftDrive(45);
  while (nPgmTime < timeMark + HEAD_FOR_CORNER_DELAY)
  {}
  stopDrive();
}
